...
export const ValibotDto = <TSchema extends BaseSchema<unknown, unknown, BaseIssue<unknown>>>(
  schema: TSchema
): {
  new (parsed: InferOutput<TSchema>): {
    data: InferOutput<TSchema>;
  };
  schema: TSchema;
  OPENAPI_METADATA: Record<string, unknown>;
} => {
  class ValibotDtoMixin<S extends BaseSchema<unknown, unknown, BaseIssue<unknown>>> {
